...
public class LoginController {
	...
	public Result<JSONObject> getUserInfo(HttpServletRequest request){
		...
		if(oConvertUtils.isNotEmpty(username)) {
			...
			try {
				queryUserDep.eq(SysUserDepart::getUserId, sysUser.getId());
				List<String> depIdList = new ArrayList<>();
				List<SysUserDepart> userDepList = sysUserDepartService.list(queryUserDep);
				if(userDepList != null && userDepList.size() > 0) {
					...
					if(depList != null && depList.size() > 0) {
						...
						for(SysDepart depart : depList) {
							depStr += depart.getDepartName() + ",";
						}
						depStr= depStr.substring(0, depStr.length() - 1);
						sysUser.setOrgCodeTxt(depStr);
					}
				}
			}catch(Exception e) {
				e.fillInStackTrace();
